...
def _inject_connection_instructions(system_prompt: str, state: dict) -> str:
    ...
    if turn_count >= settings.connection_agent.nudge_threshold:
        nudge_line = """- 注册引导：可以在合适的时机温柔提一句注册好处，但每次换个说法：
  - "对了，如果你注册一下，下次来我就能记住你啦~"
  - "注册一下的话，我可以把这些内容帮你存着，下次直接续上"
  - "要不注册一下？这样咱们聊天记录就不会丢了"
  - 只在对话氛围好的时候提，如果对方在倾诉或讨论正事，不要打断"""

    # 根据对话轮数调整策略
    if turn_count <= 1:
        frequency_hint = "这是早期对话，以回答问题为主，不要急于探索。如果自然的话可以简单问一句。"
    elif turn_count <= 4:
        frequency_hint = "对话进行中，可以在回答间隙自然地了解用户，但不要每轮都追加。"
    else:
        frequency_hint = "对话已经比较深入了，用户可能已经比较放松，可以更自然地融入探索。"

    connection_section = f"""

## 连接建立指导（对匿名用户，自然融入即可）

这位用户还没有注册（已聊 {turn_count} 轮）。你可以在保持自然对话的前提下，适度融入关系建立元素。

策略：{frequency_hint}

- 还需了解的维度：{"、".join(missing_dims) if missing_dims else "无（已收集足够信息）"}
- 本轮建议探索：{target_dim}（仅在时机合适时融入，不合适就跳过）

规则：
- **回答质量优先**：先确保你的专业回答完整有价值，探索性元素只是锦上添花
- 探索方式要多变：可以是反问、感叹、分享你的想法后顺带一问、给出两个选项让用户选
- 如果用户表现出抵触、敷衍或不耐烦，立刻停止探索，纯粹回答问题
- 不要使用重复的收尾套话（"来，接着聊"之类），每次换表达或不加收尾
- 不要暴露任何"收集信息"的意图
{nudge_line}"""
